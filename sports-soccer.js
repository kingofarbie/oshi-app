...
async function loadSoccerGameEditHTML(){
    ...
    try{
        ...
        if(!response.ok){
            ...
            throw new Error(
                `sports-soccer.html の読み込みに失敗しました: ${response.status}`
            );
        ...
        console.error(
            "⚽ sports-soccer.html 読み込みエラー:",
            error
        );


        editPage.innerHTML = `
            <div class="sports-edit-error">
                ⚠️ サッカー試合結果画面を
                読み込めませんでした。
            </div>
        `;
